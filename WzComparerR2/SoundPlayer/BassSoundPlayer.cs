...
namespace WzComparerR2
{
    public class BassSoundPlayer : ISoundPlayer
    {
        static BassSoundPlayer()
        {
            ...
            dllPath = Path.Combine(System.Windows.Forms.Application.StartupPath, Un4seen.Bass.Utils.Is64Bit ? "x64" : "x86");
            bool success = Bass.LoadMe(dllPath);
        }
        ...
        public bool Init()
        {
            if (!inited)
            {
                try
                {
                    inited = Bass.BASS_Init(-1, 44100, BASSInit.BASS_DEVICE_DEFAULT, IntPtr.Zero);
