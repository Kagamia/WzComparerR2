...
namespace WzComparerR2
{
    public class Gear : ItemBase
    {
        ...
        public bool HasTuc { get; private set; }
        public bool CanPotential { get; private set; }

        public bool FixLevel { get; private set; }
        public List<GearLevelInfo> Levels { get; private set; }
        public List<GearSealedInfo> Seals { get; private set; }
        ...
        public static Gear CreateFromNode(Wz_Node node)
        {
            Gear gear = new Gear();
            int value;
            if (node == null
                ...
                foreach (Wz_Node subNode in infoNode.Nodes)
                {
                    switch (subNode.Text)
                    {
                        case "icon":
                            ...
                            {
                                gear.Icon = new BitmapOrigin(subNode);
                            }
                            break;
                        ...
                        case "iconRaw":
                            if (subNode.Value is Wz_Png)
                            {
                                gear.IconRaw = new BitmapOrigin(subNode);
                            }
                            break;
                        ...
                        case "option": //附加潜能信息
                            Wz_Node itemWz = PluginBase.PluginManager.FindWz(Wz_Type.Item);
                            if (itemWz == null || (itemWz = itemWz.FindNodeByPath("ItemOption.img", true)) == null)
                                break;
                            int optIdx = 0;
                        ...
                        case "sealed": //封印解除信息
                            ...
                            if (sealedInfo != null)
                            {
                                ...
                                {
                                    GearSealedInfo info = GearSealedInfo.CreateFromNode(levelInfoNode);
                                    int lv;
                                    Int32.TryParse(levelInfoNode.Text, out lv);
                                    info.Level = lv;
                                    gear.Seals.Add(info);
